...
The registry intentionally includes only the models discussed in the thesis so the
Streamlit ML Lab, results tables, and written methodology stay aligned.
"""
...
def get_model_registry(task_type):
    """Return thesis-aligned models for the selected ML task."""
    if task_type == "regression":
        return {
            ...
            "MLP Regressor": mlp_regressor_pipeline(),
        }
    ...
    return {
        ...
        "MLP Classifier": mlp_classifier_pipeline(),
    }


def evaluate_models(model_registry, x_train, x_test, y_train, y_test, task_type, run_cv=False, cv_folds=5):
    """Train every model and return metrics, fitted models, and predictions."""
    evaluation_rows = []
    trained_models = {}
    prediction_details = {}
    ...
    results_df = results_df.sort_values("R2" if task_type == "regression" else "F1", ascending=False)
